...
async function displayLockers(latitude, longitude) {
    ...
    try {
        ...
        console.log("displayLockers: First locker object:", lockers[0]);
        ...
        lockers.forEach(locker => {
            ...
            console.log(`displayLockers: Appended list item for locker ${locker.locker_id}. HTML:`, listItem.outerHTML);
        });
        
        // Add event listeners for the "Select Locker" button
        lockerListElement.querySelectorAll('.select-button').forEach(button => {
            // Only add the listener if the button is not disabled
            if (!button.disabled) {
                button.addEventListener('click', (event) => {
                    // Log to confirm the event listener is working
                    console.log(`Button for locker ${event.target.dataset.lockerId} clicked!`);
...
function toggleLockerDetails(listItem) {
    ...
    if (details.classList.contains('expanded')) {
        arrowIcon.textContent = '▼'; // Point down when expanded
    } else {
        arrowIcon.textContent = '▶'; // Point right when collapsed
    }
}
...
function reserveLocker(lockerId) {
    ...
    // Send the selected locker ID to the parent window (the checkout page)
    if (window.parent) {
        window.parent.postMessage({
            type: 'lockerSelected',
            lockerId: lockerId
        }, window.parent.origin); // Ensure this matches the origin of the checkout page
    } else {
        console.log("No parent window found. Cannot send message.");
    }

    // You can add a visual confirmation here for the user
    alert(`Locker ${lockerId} has been selected for delivery!`);
}
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                ...
                alert(`Geolocation failed. Error Code: ${error.code}. Message: ${error.message}. Displaying default lockers.`);
                displayLockers(currentLat, currentLon);
            }
        );
    } else {
