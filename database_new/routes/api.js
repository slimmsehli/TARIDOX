...
const { generate4DigitCode } = require('../utils/codeGenerator'); // New import
const { sendEmail } = require('../utils/emailSender'); // New import
...
router.post('/reserve_box', async (req, res) => {
    ...
    try {
        ...
        if (locker.fullness === 'full') {
            currentDb.close();
            return res.status(400).json({ success: false, message: `Locker ${locker.name} is currently full.` });
        }

        // 3. Find Suitable Empty Box
        const emptyBoxes = await new Promise((resolve, reject) => {
            currentDb.all('SELECT * FROM locker_boxes WHERE locker_id = ? AND status = "empty"', [locker_id], (err, rows) => {
                if (err) reject(err);
                else resolve(rows);
            });
        });
