    ...
    return new Promise((resolve, reject) => {
        const db = new sqlite3.Database(DB_PATH, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
            ...
            console.log('Connected to the SQLite database.');
						...
						//check if tables exists  : ecommrce
						db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='ecommerces'", (err, row) => {
							if (err) {
                    console.error('Error checking for ecommerces table:', err.message);
                    reject(err);
                    return;
                }
						});
						
						//check if tables exists  : parcels
						db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='parcels'", (err, row) => {
							if (err) {
                    console.error('Error checking for parcels table:', err.message);
                    reject(err);
                    return;
                }
						});
